...
<html>
	...
	<body>
		...
		<section>
			...
			<p class="price">￥<span>${xhGoods.price}-${xhGoods.maxPrice}</span><span>${xhGoods.title}</span><span>下单送<m>${xhGoods.integral}</m>积分</span></p>
			<p class="yunf">运费：<span>免运费</span></p>
			<!-- <p class="select">选择：规格参数</p> -->
			<p class="select">商品描述</p>
			<div id="description">${xhGoods.description}</div>
		</section>
		...
		<div class="bottom">
			...
			<button id="buy" data-toggle="modal" data-target="#div1">立刻购买</button>
			<button id="addcar" data-toggle="modal" data-target="#div2">加入购物车</button>
		</div>
<!-----------------------------------------------------------------------------------------------------------------  -->
<!--弹出窗立即购买  -->
		<!-- <div class="back">
		...
		<!--规格属性-->
			<div class="sys_item_spec">
				<form action="monthNextLoad" method="post" id="form1" onsubmit="return cc()">
				 <input hidden="hidden" value="${xhGoods.id}" name="gid"> 
				 
				 <c:if test="${!empty monthTime}">
				<dl class="clearfix iteminfo_parameter sys_item_specpara">
					<dt>包月时间：</dt>
					<dd>
						<c:forEach items="${monthTime}" var="monthTime">
						<label><input class="r1" name="mid" type="radio" price="${monthTime.mPrice}"   value="${monthTime.id}" bmonth="${monthTime.mTime}"  />${monthTime.mTime}个月</label>
						</c:forEach>
					</dd>
				</dl>
				</c:if>
				...
				<dl class="clearfix iteminfo_parameter sys_item_specpara">
					...
					<dd>
						...
						<label><input name="rid" price="0" type="radio" value="${xhRules.id}" class="" />${xhRules.rule}</label>
						</c:forEach>
					</dd>
				</dl>
				</c:if>
...
<!-- --------------------------------------------------------------------------------------------------------------- -->
<!-- <div id="div2"> -->
		<div class="div2 modal fade" id="div2" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					<div class="modal-dialog">
		<div class="modal-content">
