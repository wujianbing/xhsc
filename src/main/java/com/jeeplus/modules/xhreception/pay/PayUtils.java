...
import com.jeeplus.common.utils.SSLUtils;
...
public class PayUtils {
	...
	//重定向地址
	public static final String redirect_uri = ReadResource.redirect_uri;
	//退款地址
	public static final String refundUrl =  "https://api.mch.weixin.qq.com/secapi/pay/refund";
	//支付成功回调通知地址
	public static final String NOTIFY_URL = "http://xhsc.xianzhi56.com/wxpay/successful";
	//state
	...
	public static final String url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={APPID}&redirect_uri=http%3A%2F%2F{REDIRECT_URI}%2Fpay%2Fwx%2Fopenid&response_type=code&scope=snsapi_base&state=123456#wechat_redirect";
	//public static final String url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={APPID}&redirect_uri={REDIRECT_URI}&response_type=code&scope=snsapi_userinfo&state={STATE}#wechat_redirect";
	public static final String code = url.replace("{APPID}", appid).replace("{REDIRECT_URI}", redirect_uri).replace("{STATE}", state);
	...
	//new String(wxRefund.toString().getBytes(),Consts.UTF_8)
	//退款订单
	public static WxRefundData createRefund(String out_trade_no,int total_fee) throws Exception{
		...
		wxRefund.setRefund_fee(total_fee);
		wxRefund.setTotal_fee(total_fee);
		wxRefund.setRefund_fee_type(fee_type);
		wxRefund.sign();
		String result = SSLUtils.doRefund(refundUrl, wxRefund.toString());
		LOGGER.info("result={}",result);
		DOMParser domParser = new DOMParser();
		WxRefundData data = domParser.getData(result);
		LOGGER.info("weixin order result after parse={}", data);
		return data;
	}
}
