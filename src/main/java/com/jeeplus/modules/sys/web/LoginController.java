...
//		System.out.println("==========================b");
	...
	public String home(HttpServletRequest request, HttpServletResponse response, Model model) throws IOException {
		String allOrder = orderService.getAllOrderNum();
		String dOrder = orderService.getDOrderNum();
		String eOrder = orderService.getEOrderNum();
		String orderAmount = orderService.getOrderAmount();
		List<XhOrder> eOrderAmounts = orderService.getEOrderAmount();
		BigDecimal f = orderService.todayAmount();
			if(f == null){
				f=new BigDecimal(0.00);
			}
		List list1 = new ArrayList<>();
		List list2 = new ArrayList<>();
		for(int i=eOrderAmounts.size();i>0;i--){
			list1.add("'"+DateUtils.formatDate(eOrderAmounts.get(i-1).getCreateDate(),"MM-dd")+"'");
			list2.add(eOrderAmounts.get(i-1).getAmount());
		}
		String goodsNum = xhGoodsService.getNum();
		model.addAttribute("a", goodsNum);
		model.addAttribute("b", allOrder);
		model.addAttribute("c", dOrder);
		model.addAttribute("d", eOrder);
		model.addAttribute("e", orderAmount);
